/**
 * API Route: /api/igdl
 * Instagram Downloader (Indown & Snapsave Fallback)
 */
...
export default async function handler(req, res) {
    ...
    if (!targetUrl) {
        return res.status(400).json({ status: false, error: 'URL Instagram wajib disertakan!' });
    }

    try {
        const downloadResult = await igdl(targetUrl);
