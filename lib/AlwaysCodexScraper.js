/**
 * AlwaysCodexScraper.js (Modified with Private Proxy Pool)
 */
...
class AlwaysCodexScraper {
  constructor() {
    ...
    // Daftar proxy privat kamu langsung disematkan di sini
    this.myProxies = [
  ...
  // ── Ambil langsung dari list proxy privat kamu ────────────────
  async loadProxyList() {
    return this.myProxies;
  }

  // ── Cari proxy yang bekerja (mendukung format user:pass) ──────
  async findWorkingProxy() {
    ...
    for (const pStr of list) {
      try {
        ...
        // Format ProxyAgent dengan autentikasi http://user:pass@ip:port
        ...
        // Lanjut ke proxy berikutnya jika gagal/timeout
      }
    }
    return null;
  }

  // ── Buat email temp via mail.tm ───────────────────────────────
  async createMailTm() {
  ...
  // ── Poll inbox & extract magic link ──────────────────────────
  async pollInbox(mailToken, timeoutSec = 90) {
  ...
  // ── Task Management untuk Express Endpoint ────────────────────
  createTask() {
    const taskId = rnd(10);
